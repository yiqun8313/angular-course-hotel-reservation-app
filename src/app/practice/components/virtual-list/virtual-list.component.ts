...
export class VirtualListComponent {
  ...
  updateVirtual(event: Event): void {
    this.virtualStart = Math.floor((event.currentTarget as HTMLElement).scrollTop / 40);
  }

  get virtualVisible(): number[] {
    return this.virtualRows.map((_, index) => this.virtualStart + index).filter(index => index < 10000);
  }
}
